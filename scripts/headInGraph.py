...
class headInGraph():
    ...
    def printGraphViz(self, out, title, outPutExternal):
        out.write("digraph G  { \n")
        ...
        for nPos in range(len(self.nodes_)):
            if(not outPutExternal and self.nodes_[nPos].type_ == "external"):
                continue
            out.write(self.nodes_[nPos].value_ + "[shape=circle,style=filled,fixedsize =false, color = \"#000000\", fillcolor =\"" + self.nodes_[nPos].color_ + "\", width = " + str( self.nodes_[nPos].filesize_) + "]\n") 
        for nPos in range(len(self.nodes_)):
            if(not outPutExternal and self.nodes_[nPos].type_ == "external"):
                continue
